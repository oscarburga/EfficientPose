        ...
        if batch_num % 20 == 0:
            ...
            for i, image_coordinates in enumerate(coordinates):
                for body_part, body_part_x, body_part_y in image_coordinates:
                    cordsDict[body_part] = (body_part_x,body_part_y)
            # print(batch_num)
            # print(cordsDict)
...
def showCords(video, file_path, coordinates):
    for i, image_coordinates in enumerate(coordinates):
...
def main(file_path, model_name, framework_name, visualize, store, isdir):
    """
    ...
    # MrMime2: Process directory
    import os
    if isdir:
        this_path = os.path.dirname(__file__)
        if file_path not in os.listdir(this_path):
            os.mkdir(input_path)
        if not os.path.isdir(file_path):
            return
        listed_dir = os.listdir(file_path)
        for file in listed_dir:
            ...
            if os.path.isfile(full_path):
                ...
                tracktypes = [track.track_type for track in MediaInfo.parse(full_path).tracks]
                if 'Video' in tracktypes:
                    print(f"Processing video {full_path}. This could take a few minutes depending on video size and length.")
                    perform_tracking(video=True, file_path=normpath(full_path), model_name=model_name, framework_name=framework_name, visualize=visualize, store=store)
                elif 'Image' in tracktypes:
                    print(f'Processing image {full_path}...')
                    perform_tracking(video=False, file_path=normpath(full_path), model_name=model_name, framework_name=framework_name, visualize=visualize, store=store)
        return
...
if __name__== '__main__':
    ...
    long_options = ['path=', 'model=', 'framework=', 'visualize', 'store', 'dir']
    try:
    ...
    main(file_path=file_path, model_name=model_name, framework_name=framework_name, visualize=visualize, store=store, isdir=isdir)
